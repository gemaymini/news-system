    ...
    let columns = [
        {
            ...
            align: 'center',
            render: (text, record) => (
                <Link to={`/userinfo/${record.id}`}>{text}</Link> // 点击跳转到用户简介页面
            ),
        },
        {
            title: '性别',
        ...
        },
        {
            ...
            align: 'center',
            render: (text, record) =>
                AdminStore.modules.operations.includes('userState') ? (
                    <Switch
                        disabled={record.character_id === 1}
                        checkedChildren="正常"
                        unCheckedChildren="已停用"
                        checked={text}
                        onChange={(checked) => {
                            changeState(checked, record.id);
                        }}
                    />
                ) : (
        ...
        },
        {
            ...
            render: (text, record) => {
                if (record.character_id === 1) {
                    ...
                    return (
                        <span>
                            {AdminStore.modules.operations.find((item) => item === 'userUpdate') && (
                                <Tooltip title="更改角色">
                                    <Button
                                        shape="circle"
                                        type="primary"
                            ...
                            {AdminStore.modules.operations.find((item) => item === 'userDelete') && (
                                <Tooltip title="删除">
                                    <Button
                                        shape="circle"
                                        danger
    ...
    return (
        <div>
            <AddModal
                ...
                close={() => {
                    UserStore.setIsAddVisible(false);
                }}
                getUserList={() => {
            ...
            <EditModal
                ...
                close={() => {
                    UserStore.setIsEditVisible(false);
                }}
            ...
            <SearchComponent
                addUser={() => {
                    UserStore.setIsAddVisible(true);
                }}
                search={(values) => {
            ...
            <Table
                rowKey={(item) => item.id}
                columns={
                    AdminStore.modules.operations.some((item) => item === 'userDelete' || item === 'userUpdate')
                        ? columns
                        : columns.slice(0, 6)
                }
                ...
                pagination={{
                    ...
                    onChange: (value) => {
                        UserStore.setCurrentPage(value);
                    },
